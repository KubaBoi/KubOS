...
namespace AHCI{
    ...
    bool Port::Read(uint64_t sector, uint32_t serctorCount, void* buffer) {
        ...
        cmdHeader->write = 0; //this is read
        ...
        commandTable->prdtEntry[0].byteCount = (serctorCount << 9) - 1; //512 bytes per sector
        ...
        cmdFIS->countLow = serctorCount & 0xFF;
        cmdFIS->countHigh = (serctorCount >> 8) & 0xFF;
    ...
    AHCIDriver::AHCIDriver(PCI::PCIDeviceHeader* pciBaseAddress){
        ...
        for (int i = 0; i < portCount; i++) {
            ...
            for (int i = 0; i < 1024; i++) {
                GlobalRenderer->putChar(port->buffer[i]);
            }
            GlobalRenderer->Next();
        }
    }
